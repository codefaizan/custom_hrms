...
class SalarySlip(TransactionBase):
	...
	def joining_date(self):
		if not hasattr(self, "cached_joining_date"):
			self.cached_joining_date = frappe.get_cached_value(
				"Employee",
				self.employee,
				"date_of_joining",
			)

		return self.cached_joining_date

	@property
	def relieving_date(self):
		if not hasattr(self, "cached_relieving_date"):
			self.cached_relieving_date = frappe.get_cached_value(
				"Employee",
				self.employee,
				"relieving_date",
			)

		return self.cached_relieving_date

	@property
	def payroll_period(self):
		if not hasattr(self, "cached_payroll_period"):
			self.cached_payroll_period = get_payroll_period(self.start_date, self.end_date, self.company)

		return self.cached_payroll_period

	@property
	def actual_start_date(self):
		if not hasattr(self, "cached_actual_start_date"):
			self.cached_actual_start_date = self.start_date

			if self.joining_date and getdate(self.start_date) < self.joining_date <= getdate(self.end_date):
				self.cached_actual_start_date = self.joining_date

		return self.cached_actual_start_date

	@property
	def actual_end_date(self):
		if not hasattr(self, "cached_actual_end_date"):
			self.cached_actual_end_date = self.end_date

			if self.relieving_date and getdate(self.start_date) <= self.relieving_date < getdate(
				self.end_date
			):
				self.cached_actual_end_date = self.relieving_date

		return self.cached_actual_end_date
		...
		if self.employee:
			...
			if struct:
				...
				self.salary_slip_based_on_timesheet = (
					self.salary_structure_doc.salary_slip_based_on_timesheet or 0
				)
				self.set_time_sheet()
	...
	def pull_sal_struct(self):
		...
		if self.salary_slip_based_on_timesheet:
			self.salary_structure = self.salary_structure_doc.name
			self.hour_rate = self.salary_structure_doc.hour_rate
			self.base_hour_rate = flt(self.hour_rate) * flt(self.exchange_rate)
			self.total_working_hours = sum([d.working_hours or 0.0 for d in self.timesheets]) or 0.0
			wages_amount = self.hour_rate * self.total_working_hours

			self.add_earning_for_hourly_wages(self, self.salary_structure_doc.salary_component, wages_amount)
	...
	def get_working_days_details(self, lwp=None, for_preview=0):
		...
		if flt(payment_days) > flt(lwp):
			...
			if payroll_settings.payroll_based_on == "Attendance":
				self.payment_days = self.payment_days - flt(absent)
			...
			if (
				...
				self.absent_days = self.absent_days + (
					unmarked_days + half_absent_days * daily_wages_fraction_for_half_day
				)  # will be treated as absent
				self.payment_days = self.payment_days - (unmarked_days + half_absent_days * daily_wages_fraction_for_half_day)
		else:
			self.payment_days = 0
	...
	def get_unmarked_days(
		...
		unmarked_days = (
			...
			- self.get_days_outside_period(include_holidays_in_total_working_days, holidays)
			- self.get_marked_attendance_days(holidays)
		)

		if include_holidays_in_total_working_days and holidays:
			unmarked_days = unmarked_days - self.get_number_of_holidays(holidays)
	...
	def get_days_outside_period(
		...
		if self.actual_start_date != self.start_date:
			days = days + get_days(self.start_date, add_days(self.joining_date, -1))

		if self.actual_end_date != self.end_date:
			days = days + get_days(add_days(self.relieving_date, 1), self.end_date)

		return days

	def get_number_of_holidays(self, holidays: list | None = None) -> float:
		no_of_holidays = 0
	...
	def get_marked_attendance_days(self, holidays: list | None = None) -> float:
		Attendance = frappe.qb.DocType("Attendance")
		query = (
			frappe.qb.from_(Attendance)
			.select(Count("*"))
	...
	def get_leave_type_map(self) -> dict:
		...
		return frappe.cache().get_value(LEAVE_TYPE_MAP, get_leave_type_map_data)
	...
	def set_salary_structure_assignment(self):
		self.salary_structure_assignment = frappe.db.get_value(
	...
	def get_future_period_non_taxable_earnings(self):
		...
		for earning in salary_slip.earnings:
			if not earning.is_tax_applicable and not earning.additional_salary:
				future_period_non_taxable_earnings = future_period_non_taxable_earnings + earning.amount
	...
	def get_non_taxable_earnings_for_current_period(self):
		...
		for earning in self.earnings:
			...
			if earning.additional_amount:
				non_taxable_additional_salary = non_taxable_additional_salary + earning.additional_amount

				# Future recurring additional salary
				if earning.additional_salary and earning.is_recurring_additional_salary:
					non_taxable_additional_salary = non_taxable_additional_salary + self.get_future_recurring_additional_amount(
						earning.additional_salary, earning.additional_amount
					)
			else:
				current_period_non_taxable_earnings = current_period_non_taxable_earnings + earning.amount
	...
	def compute_annual_deductions_before_tax_calculation(self):
		...
		for d in self.get("deductions"):
			if d.exempted_from_income_tax:
				current_period_exempted_amount = current_period_exempted_amount + d.amount

		# Future period exempted amount
		for deduction in self.salary_structure_doc.get("deductions"):
			if deduction.exempted_from_income_tax:
				if deduction.amount_based_on_formula:
					for sub_period in range(1, ceil(self.remaining_sub_periods)):
						future_period_exempted_amount = future_period_exempted_amount + self.get_amount_from_formula(deduction, sub_period)
				else:
					future_period_exempted_amount = future_period_exempted_amount + deduction.amount * (ceil(self.remaining_sub_periods) - 1)
	...
	def get_income_tax_deducted_till_date(self):
		tax_deducted = 0.0
		for tax_component in self.get("component_based_variable_tax") or {}:
			tax_deducted = tax_deducted + (
				self.component_based_variable_tax[tax_component]["previous_total_paid_taxes"]
				+ self.component_based_variable_tax[tax_component]["current_tax_amount"]
			)
		return tax_deducted
	...
	# 	if (
	# 		self.salary_slip_based_on_timesheet
	# 		and struct_row.salary_component == self.salary_structure_doc.salary_component
	# 	):
	# 		return
		...
		if payroll_period:
			if getdate(payroll_period.end_date) <= getdate(self.end_date):
				last_benefits = get_last_payroll_period_benefits(
					self.employee, self.start_date, self.end_date, payroll_period, self.salary_structure_doc
				)
				if last_benefits:
	...
	def add_tax_components(self):
		...
		for d in self.salary_structure_doc.get("deductions"):
		...
		self.component_based_variable_tax = {}
		for d in tax_components:
			self.component_based_variable_tax.setdefault(d, {})
